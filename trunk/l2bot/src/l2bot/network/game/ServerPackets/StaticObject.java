...
public class StaticObject extends L2GameServerPacket
{
    ...
    {
        //writeC(0x9f);
        int sObjId = readD();//writeD(_staticObjectId);
        int objId = readD();//writeD(_objectId);
        int type = readD();//writeD(_type);
        boolean  targetable = readD() != 0;//writeD(_isTargetable ? 1 : 0);
        int mesh = readD();//writeD(_meshIndex);
        boolean closed = readD() != 0;//writeD(_isClosed ? 1 : 0);
        boolean enemy = readD() != 0;//writeD(_isEnemy ? 1 : 0);
        int hp = readD();//writeD(_currentHp);
        int maxHp = readD();//writeD(_maxHp);
        boolean showHp = readD() != 0;//writeD(_showHp ? 1 : 0);
        int damege = readD();//writeD(_damageGrade);
    }
