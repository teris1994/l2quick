...
//
	...
	public void readP()
	{
		...
		int msgId = readD();//writeD(_messageId);
                String msg;
                try{
                    msg=ResourceBundle.getBundle("lang/sysmsgs").getString(String.valueOf(msgId));
                    if(msg.length() < 3){return;}
                }catch(Exception e){
                    return;
                }
                
		int sz = readD();//writeD(_types.size());

		for (int i=0;i<sz;i++)
		{
			//int t = _types.get(i).intValue();

			int t = readD();//writeD(t);

			switch (t)
			{
				case TYPE_TEXT:
				//{
					msg.replaceFirst("$c"+(i+1), readS());//writeS( (String)_values.get(i));
					break;
				//}
				case TYPE_NUMBER:
                                        msg.replaceFirst("$c"+(i+1), String.valueOf(readD()));
                                        break;
				case TYPE_NPC_NAME:
                                        //implementar la data del npc!!
                                        break;
				case TYPE_ITEM_NAME:
				//{
                                        msg.replaceFirst("$c"+(i+1), DataHandler.getItem(readD()).name);
				...
				//{
                                                //implementar la data de las skills!!
					//int t1 = ((Integer)_values.get(i)).intValue();
					//writeD(t1); // Skill Id
				...
				case TYPE_ZONE_NAME:
				//{
                                                //implementar la data de las zonas!!!
