...
public final class AttackRequest extends L2GameClientPacket
{
	...
	////@Override
	protected void readImpl(int objectId,int x, int y, int z, boolean shilft)
	{
		writeD(objectId);//_objectId  = readD();
		writeD(x);//_originX  = readD();
		writeD(y);//_originY  = readD();
		writeD(z);//_originZ  = readD();
		writeC(shilft?1:0);//_attackId  = readC(); 	 // 0 for simple click   1 for shift-click
	}
