...
public final class RequestWearItem extends L2GameClientPacket
{
   ...
   /* class RemoveWearItemsTask implements Runnable
     ...
     */
    //@Override
	protected void readImpl(int list, int[] ids)
    {
        // Read and Decrypt the RequestWearItem Client->Server Packet
    	//_activeChar = getClient().getActiveChar();
        writeD(0);//_unknow = readD();
        writeD(list);//_listId = readD(); // List of ItemID to Wear
        writeD(ids.length);//_count = readD();  // Number of Item to Wear
