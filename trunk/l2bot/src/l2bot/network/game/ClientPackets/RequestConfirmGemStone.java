...
//import *net.sf.l2j.gameserver.serverpackets.ExPutCommissionResultForVariationMake;
...
public final class RequestConfirmGemStone extends L2GameClientPacket
{
	 ...
	 */
	////@Override
	protected void readImpl(int itemid, int refinerid, int gemstoneid,int gemstonecount)
	{
		writeD(itemid);//_targetItemObjId = readD();
		writeD(refinerid);//_refinerItemObjId = readD();
		writeD(gemstoneid);//_gemstoneItemObjId = readD();
		writeD(gemstonecount);//_gemstoneCount= readD();
	}
