...
*/
	...
	////@Override
	protected void readImpl(int tx, int ty, int tz, int ox, int oy, int oz, boolean mouse)
	{
		writeD(tx);//_targetX  = readD();
		writeD(ty);//_targetY  = readD();
		writeD(tz);//_targetZ  = readD();
		writeD(ox);//_originX  = readD();
		writeD(oy);//_originY  = readD();
		writeD(oz);//_originZ  = readD();
		//try
		//{
		writeD(mouse?1:0);//	_moveMovement = readD(); // is 0 if cursor keys are used  1 if mouse is used
		//}
