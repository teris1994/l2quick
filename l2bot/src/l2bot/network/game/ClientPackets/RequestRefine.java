...
public final class RequestRefine extends L2GameClientPacket
{
	...
	//@Override
	protected void readImpl(int target, int refiner, int gemstoneid,int gemcount)
	{
		writeD(target);//_targetItemObjId = readD();
		writeD(refiner);//_refinerItemObjId = readD();
		writeD(gemstoneid);//_gemstoneItemObjId = readD();
		writeD(gemcount);//_gemstoneCount = readD();
	}
