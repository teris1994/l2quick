...
public final class RequestExMagicSkillUseGround extends L2GameClientPacket
{
  ...
  //  private boolean _shiftPressed;
   ...
   // //@Override
    protected void readImpl(int x, int y, int z, int skill, boolean control, boolean shilft)
    {
        writeD(x);//_x = readD();
        writeD(y);//_y = readD();
        writeD(z);//_z = readD();
        writeD(skill);//_skillId = readD();
        writeD(control?1:0);//_ctrlPressed = readD() != 0;
        writeD(shilft?1:0);//_shiftPressed = readC() != 0;
    }
